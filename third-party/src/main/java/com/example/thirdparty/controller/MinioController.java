...
import com.example.thirdparty.dto.response.FileUploadResponse;
import com.example.thirdparty.utils.MinioUtil;
...
public class MinioController {
     ...
     */
    @PostMapping("/upload")
    public R upload(@RequestParam(name = "file", required = false) MultipartFile file, @RequestParam(required = false) String bucketName) {
    ...
    public ResponseEntity<byte[]> downloadToLocal(@PathVariable("objectName") String objectName, HttpServletResponse response) throws Exception {
        ...
        try {
            // 获取"myobject"的输入流。
            stream = minioUtil.getObject(defaultBucket, objectName);
            if (stream == null) {
